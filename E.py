import gpiod
import time
import math
from PIL import Image, ImageDraw, ImageFont
from luma.core.interface.serial import spi
from luma.lcd.device import st7735

# --- GPIO setup using libgpiod on main chip ---
chip = gpiod.Chip("gpiochip0")  # main GPIO controller

RST_PIN = 27
DC_PIN = 25

# Setup lines
rst_config = gpiod.LineSettings()
rst_config.direction = gpiod.LineDirection.OUTPUT
dc_config = gpiod.LineSettings()
dc_config.direction = gpiod.LineDirection.OUTPUT

lines_config = {RST_PIN: rst_config, DC_PIN: dc_config}
lines = chip.request_lines(lines_config, consumer="lcd")

# Access lines
rst = lines[RST_PIN]
dc = lines[DC_PIN]

# Reset pulse
rst.set_value(0)
time.sleep(0.1)
rst.set_value(1)

# --- SPI display setup (no GPIO args needed here) ---
serial = spi(port=0, device=0, bus_speed_hz=16000000)
device = st7735(serial, width=160, height=80, rotate=0)

font = ImageFont.load_default()

def fade_color(t):
    return (
        int((math.sin(t) * 127) + 128),
        int((math.sin(t + 2) * 127) + 128),
        int((math.sin(t + 4) * 127) + 128),
    )

def launch_animation():
    for t in range(200):
        img = Image.new("RGB", device.size, fade_color(t / 10))
        draw = ImageDraw.Draw(img)
        radius = int(20 + 10 * math.sin(t / 5))
        x, y = device.width // 2, device.height // 2
        draw.ellipse((x - radius, y - radius, x + radius, y + radius),
                     outline="white", width=2)
        text = "Rozaeta"
        w, h = draw.textsize(text, font)
        draw.text(((device.width - w) / 2, (device.height - h) / 2),
                  text, font=font, fill="white")
        device.display(img)
        time.sleep(0.05)

    img = Image.new("RGB", device.size, "black")
    draw = ImageDraw.Draw(img)
    draw.text((device.width / 2 - 25, device.height / 2 - 5),
              "Rozaeta", font=font, fill="white")
    device.display(img)

if __name__ == "__main__":
    device.clear()
    launch_animation()
